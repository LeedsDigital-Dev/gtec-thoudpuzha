...
## Corpus Check
- 186 files · ~47,888 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 797 nodes · 1381 edges · 53 communities (43 shown, 10 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 1 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `35750447`
...
1. `requireRole()` - 71 edges
2. `logAdminAction()` - 48 edges
3. `Role` - 40 edges
4. `cn()` - 19 edges
5. `Button()` - 16 edges
...
Cohesion: 0.07
Nodes (58): createPartner(), deletePartner(), localeFromFormData(), movePartner(), updatePartner(), PageProps, createCategory(), createCourse() (+50 more)
...
Cohesion: 0.05
Nodes (29): AuditLogPage(), AuditLogPageProps, EnquiriesPage(), EnquiriesPageProps, RegistrationResult, submitEmployerRegistration(), mockAuth, mockClerkClient (+21 more)
...
Cohesion: 0.06
Nodes (46): EMPLOYEE_RANGES, SECTORS, Step, BiodataActionResult, BiodataFormData, saveBiodata(), BiodataPage(), BiodataForm() (+38 more)
...
- **Why does `Role` connect `FlashNewsBar.tsx` to `news-events.test.ts`, `BiodataForm.test.tsx`, `students/actions.test.ts`, `actions.test.ts`, `FlashNewsBar.tsx`, `assignments/page.tsx`, `gallery/actions.ts`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `requireRole()` connect `FlashNewsBar.tsx` to `FlashNewsBar.tsx`, `package.json`, `BiodataForm.test.tsx`, `students/actions.test.ts`, `actions.test.ts`, `assignments/page.tsx`, `gallery/actions.ts`?**
