    ...
    <main className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-foreground">{record.fullName}</h1>
      ...
      <section className="rounded-lg border border-border bg-card p-5 space-y-4 shadow-xs">
        ...
        <div className="grid gap-4 sm:grid-cols-2 text-sm">
          <div>
            ...
            <span className="font-medium text-foreground">{record.fullName}</span>
          </div>
          <div>
            <span className="text-muted-foreground block text-xs font-medium">Student ID</span>
            <span className="font-mono text-foreground">{record.studentId}</span>
          </div>
          <div>
          ...
          <div>
            ...
            <span className="font-mono text-foreground break-all">{record.email || "Not provided"}</span>
          </div>
          <div>
          ...
          <div>
            ...
            <span className="font-mono text-xs text-foreground break-all">{record.linkedUserId || "None"}</span>
          </div>
        </div>
      </section>
          ...
          <section className="rounded-lg border border-border bg-card p-5 space-y-4 shadow-xs">
            ...
            <div className="grid gap-4 sm:grid-cols-2 text-sm">
              <div>
                ...
                <span className="text-foreground">{candidate.educationalQualification || "—"}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="text-foreground">{candidate.preferredJobLocation || "—"}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="text-foreground">{candidate.languagesKnown.join(", ") || "—"}</span>
              </div>
            </div>
            {candidate.careerObjective && (
              <div className="pt-2 border-t text-sm">
                <span className="text-muted-foreground block text-xs font-medium">Career Objective</span>
                <p className="mt-1 text-foreground">{candidate.careerObjective}</p>
              </div>
            )}
          </section>
            ...
            {candidate.enrollments.length > 0 ? (
              <div className="space-y-2">
                {candidate.enrollments.map((e) => (
                  <div key={e.id} className="flex justify-between items-center p-3 rounded border border-border bg-muted/20 text-sm">
                    <span className="font-medium text-foreground">{e.course.titleEn}</span>
                    <span className="text-xs text-muted-foreground">Enrolled: {new Date(e.enrolledAt).toLocaleDateString()}</span>
                  </div>
                ))}
              </div>
            ) : (
      ...
      <div className="pt-4 flex justify-between items-center">
        <Link href={`/${locale}/admin/students`}>
          <Button variant="outline">← Back to All Students</Button>
        </Link>
        {candidate && (
          <Link href={`/${locale}/admin/students/course-enrollment?studentProfileId=${encodeURIComponent(candidate.id)}`}>
            <Button>Manage Course Enrollments →</Button>
          </Link>
        )}
      </div>
    </main>
