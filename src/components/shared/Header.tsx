...
export function Header({
            ...
            {isSignedIn ? (
              ...
              <div className="flex flex-col gap-2.5">
                ...
                <Link
                  ...
                  aria-label={t("login")}
                  onClick={() => setDrawerOpen(false)}
                  className="flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground shadow-xs hover:bg-primary/90 transition-all active:scale-[0.99] mt-1"
                >
                  <User className="size-3.5" />
                  <span>{t("login")}</span>
                </Link>
  ...
  return (
    <>
      ...
      >
        <div className="mx-auto flex h-20 sm:h-22 max-w-[1440px] items-center justify-between px-4 sm:px-6 lg:px-8 xl:px-10">
          {/* Brand Logo */}
          <Link
            href={localeHref("/")}
